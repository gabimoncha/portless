...
> **Expo / React Native on iOS:** iOS App Transport Security (ATS) blocks cleartext HTTP to `.localhost` subdomains by default. Add an exception domain to your `app.json`:
>
> ```json
> {
>   "expo": {
...
> ```
>
> For [LAN mode](/commands#lan-mode) (`.local` domains), use `NSAllowsLocalNetworking` instead.
